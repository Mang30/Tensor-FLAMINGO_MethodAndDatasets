#!/usr/bin/env python3
"""Prepare and evaluate raw-distance LRTC inputs for simulation_multicondition."""
...
def prepare_one(record: RawDataset, input_root: Path, force: bool = False) -> Path:
    ...
    matrix_dir = dataset_input / "distance_matrices"
    ...
    for cell_idx, row in metadata.iterrows():
        src = Path(row["sparse_distance_path"])
        ...
        rows.append(
            {
                ...
                "sparse_distance_path": row["sparse_distance_path"],
                "gt_distance_path": row["gt_distance_path"],
...
def evaluate_one(record: RawDataset, input_root: Path, output_root: Path, npy_name: str = "completed_tensor.npy") -> None:
    ...
    for _, row in index_df.iterrows():
        ...
        observed = load_matrix(input_dir / "distance_matrices" / row["input_file"])
        truth = load_matrix(Path(row["gt_distance_path"]))
        metrics = distance_metrics_for_cell(completed[cell_idx], observed, truth)
        rows.append(
            {
    ...
    pd.DataFrame(summary_rows).to_csv(output_dir / "distance_summary_metrics.csv", index=False)

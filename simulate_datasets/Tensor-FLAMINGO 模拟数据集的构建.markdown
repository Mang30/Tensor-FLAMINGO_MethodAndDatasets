...
Step 0: 参数配置
  ↓
Step 1: 生成/加载共识3D坐标consensus (clean template) # N种细胞类型下有 N 个共识3D 坐标
  ↓ + smooth displacement (biological variability)        ← script 2 的 perturb_structure
Step 2: single-cell GT  ← dense, 存到 gt_distance/          ⭐ 评估目标
  ↓ random mask (downsample to ~0.5%)
  ↓ + Gaussian noise on observed Ω (measurement noise)   ← add_noise_to_observed_distances
Step 3: sparse tensor (input)  ← 存到 downsampled_data/
  ↓ model
predicted dense → vs single-cell GT → PCC/MSE
``````
